...
module lumio_framework::config_buffer {
    use std::error;
    use std::string::String;
    use lumio_std::any;
    use lumio_std::any::Any;
    use lumio_std::simple_map;
    use lumio_std::simple_map::SimpleMap;
    ...
    friend lumio_framework::jwks;
    friend lumio_framework::jwk_consensus_config;
    friend lumio_framework::keyless_account;
    friend lumio_framework::randomness_api_v0_config;
    friend lumio_framework::randomness_config;
    friend lumio_framework::randomness_config_seqnum;
    ...
    public fun initialize(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        if (!exists<PendingConfigs>(@lumio_framework)) {
            move_to(lumio_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@lumio_framework)) {
            let config = borrow_global<PendingConfigs>(@lumio_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@lumio_framework);
