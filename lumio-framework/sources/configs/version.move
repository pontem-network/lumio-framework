...
module lumio_framework::version {
    use std::error;
    use std::signer;
    ...
    public(friend) fun initialize(lumio_framework: &signer, initial_version: u64) {
        system_addresses::assert_lumio_framework(lumio_framework);

        move_to(lumio_framework, Version { major: initial_version });
        // Give lumio framework account capability to call set version. This allows on chain governance to do it through
        // control of the lumio framework account.
        move_to(lumio_framework, SetVersionCapability {});
    }
    ...
    public entry fun set_version(account: &signer, major: u64) acquires Version {
        ...
        let old_major = borrow_global<Version>(@lumio_framework).major;
        assert!(old_major < major, error::invalid_argument(EINVALID_MAJOR_VERSION_NUMBER));

        let config = borrow_global_mut<Version>(@lumio_framework);
        config.major = major;
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires Version {
        ...
        if (config_buffer::does_exist<Version>()) {
            ...
            if (exists<Version>(@lumio_framework)) {
                *borrow_global_mut<Version>(@lumio_framework) = new_value;
            } else {
    ...
    public entry fun test_set_version(lumio_framework: signer) acquires Version {
        initialize(&lumio_framework, 1);
        assert!(borrow_global<Version>(@lumio_framework).major == 1, 0);
        set_version(&lumio_framework, 2);
        assert!(borrow_global<Version>(@lumio_framework).major == 2, 1);
    }

    #[test(lumio_framework = @lumio_framework, core_resources = @core_resources)]
    ...
    #[test(lumio_framework = @lumio_framework, random_account = @0x123)]
