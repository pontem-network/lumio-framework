spec lumio_framework::stake {
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (LumioCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@lumio_framework) ==> validator_set_is_valid();
        // After genesis, `LumioCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<LumioCoinCapabilities>(@lumio_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@lumio_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@lumio_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@lumio_framework))) ==>
            old(global<ValidatorSet>(@lumio_framework)) == global<ValidatorSet>(@lumio_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@lumio_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@lumio_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@lumio_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(lumio_framework: &signer) {
        ...
        let lumio_addr = signer::address_of(lumio_framework);
        aborts_if !system_addresses::is_lumio_framework_address(lumio_addr);
        aborts_if exists<ValidatorSet>(lumio_addr);
        aborts_if exists<ValidatorPerformance>(lumio_addr);
        ensures exists<ValidatorSet>(lumio_addr);
        ensures global<ValidatorSet>(lumio_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(lumio_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@lumio_framework);
        aborts_if !exists<ValidatorSet>(@lumio_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@lumio_framework);
        let post p_validator_set = global<ValidatorSet>(@lumio_framework);
    ...
    )
    {
        ...
        let bool_find_validator = !option::spec_is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::spec_is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::spec_is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@lumio_framework);
        ...
        aborts_if !exists<coin::CoinStore<LumioCoin>>(addr);
        include coin::DepositAbortsIf<LumioCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<LumioCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<LumioCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<LumioCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@lumio_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<LumioCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@lumio_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = lumio_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@lumio_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = lumio_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include lumio_framework::lumio_coin::ExistsLumioCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@lumio_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@lumio_framework);
    ...
    spec schema UpdateStakePoolAbortsIf {
        ...
        let lumio_addr = type_info::type_of<LumioCoin>().account_address;
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<LumioCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<LumioCoin>>(addr);
        modifies global<coin::CoinInfo<LumioCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<LumioCoin> { amount: amount };
    }
    ...
    spec schema GetReconfigStartTimeRequirement {
        requires exists<timestamp::CurrentTimeMicroseconds>(@lumio_framework);
        include reconfiguration_state::StartTimeSecsRequirement;
    }

    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@lumio_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let lumio = @lumio_framework;
        let pre_validator_set = global<ValidatorSet>(lumio);
        let post validator_set = global<ValidatorSet>(lumio);
        let staking_config = global<staking_config::StakingConfig>(lumio);
    ...
    spec configure_allowed_validators(lumio_framework: &signer, accounts: vector<address>) {
        let lumio_framework_address = signer::address_of(lumio_framework);
        aborts_if !system_addresses::is_lumio_framework_address(lumio_framework_address);
        let post allowed = global<AllowedValidators>(lumio_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@lumio_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<LumioCoinCapabilities>(@lumio_framework);
        requires exists<ValidatorPerformance>(@lumio_framework);
        requires exists<ValidatorSet>(@lumio_framework);
        requires exists<StakingConfig>(@lumio_framework);
        requires exists<StakingRewardsConfig>(@lumio_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@lumio_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@lumio_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = lumio_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = lumio_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate
