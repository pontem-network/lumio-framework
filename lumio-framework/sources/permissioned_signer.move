...
/// for blind signing.
module lumio_framework::permissioned_signer {
    use std::features;
    use std::signer;
    use std::error;
    use std::vector;
    use std::option::{Option, Self};
    use lumio_std::copyable_any::{Self, Any};
    use lumio_framework::big_ordered_map::{Self, BigOrderedMap};
    use lumio_framework::create_signer::create_signer;
    use lumio_framework::transaction_context::generate_auid_address;
    ...
    fun signer_address_roundtrip(
        ...
        let lumio_framework = create_signer(@0x1);
    ...
    #[test(creator = @0xcafe)]
    #[expected_failure(abort_code = 0x1C5, location = lumio_std::bcs)]
