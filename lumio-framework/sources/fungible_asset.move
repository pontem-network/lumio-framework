...
module lumio_framework::fungible_asset {
    ...
    use lumio_framework::object::{Self, Object, ConstructorRef, DeleteRef, ExtendRef};
    ...
    friend lumio_framework::primary_fungible_store;
    friend lumio_framework::lumio_account;

    friend lumio_framework::dispatchable_fungible_asset;
    ...
    const ELUM_NOT_DISPATCHABLE: u64 = 31;
    /// Flag for Concurrent Supply not enabled
    ...
    inline fun register_dispatch_function_sanity_check(
        ...
        assert!(
            object::address_from_constructor_ref(constructor_ref) != @lumio_fungible_asset,
            error::permission_denied(ELUM_NOT_DISPATCHABLE)
        );
        assert!(
    ...
    fun has_deposit_dispatch_function(metadata: Object<Metadata>): bool acquires DispatchFunctionStore {
        ...
        if(metadata_addr != @lumio_fungible_asset && exists<DispatchFunctionStore>(metadata_addr)) {
    ...
    fun has_supply_dispatch_function(metadata_addr: address): bool {
        // Short circuit on LUM for better perf
        if (metadata_addr != @lumio_fungible_asset) {
    ...
    #[test(fx = @lumio_framework, creator = @0xcafe)]
    ...
    fun test_e2e_withdraw_limit(
        ...
        let lumio_framework = account::create_signer_for_test(@0x1);
