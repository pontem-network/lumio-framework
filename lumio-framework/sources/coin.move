...
module lumio_framework::coin {
    ...
    use lumio_framework::optional_aggregator::{Self, OptionalAggregator};
    use lumio_framework::permissioned_signer;
    use lumio_framework::system_addresses;

    use lumio_framework::fungible_asset::{Self, FungibleAsset, Metadata, MintRef, TransferRef, BurnRef};
    use lumio_framework::object::{Self, Object, object_address};
    use lumio_framework::primary_fungible_store;
    use lumio_std::type_info::{Self, TypeInfo};
    use lumio_framework::create_signer;

    friend lumio_framework::lumio_coin;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@lumio_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@lumio_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        if (!exists<CoinConversionMap>(@lumio_framework)) {
            move_to(lumio_framework, CoinConversionMap {
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_lum_creation: bool): Object<Metadata> {
        ...
        assert!(exists<CoinConversionMap>(@lumio_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
            ...
            let metadata_object_cref =
                if (is_lum) {
                    ...
                    object::create_named_object(
                        &create_signer::create_signer(@lumio_fungible_asset),
    ...
    public fun allow_supply_upgrades(_lumio_framework: &signer, _allowed: bool) {
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use lumio_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @lumio_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = lumio_framework::fungible_asset)]
    ...
    #[test(framework = @lumio_framework, other = @0x123)]
    ...
    #[test(framework = @lumio_framework)]
    ...
    #[test(framework = @lumio_framework)]
    #[expected_failure(abort_code = 0x20001, location = lumio_framework::aggregator)]
    ...
    #[test(account = @lumio_framework, aaron = @0xcafe)]
    ...
    #[test(account = @lumio_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @lumio_framework, bob = @0xb0b)]
    ...
    #[test(account = @lumio_framework)]
    #[expected_failure(abort_code = 0x50024, location = lumio_framework::fungible_asset)]
