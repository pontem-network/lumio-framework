...
module lumio_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(lumio_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by LumioVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@lumio_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@lumio_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@lumio_framework).microseconds
    }
