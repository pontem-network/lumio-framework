spec lumio_framework::reconfiguration {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: The Configuration resource is stored under the Lumio framework account with initial values upon
    /// module's initialization.
    /// Criticality: Medium
    /// Implementation: The Configuration resource may only be initialized with specific values and published under the
    /// lumio_framework account.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<Configuration>(@lumio_framework);
    ...
    spec schema AbortsIfNotLumioFramework {
    ...
    spec initialize(lumio_framework: &signer) {
        ...
        use lumio_framework::guid;

        include AbortsIfNotLumioFramework;
        let addr = signer::address_of(lumio_framework);
        let post config = global<Configuration>(@lumio_framework);
        requires exists<Account>(addr);
        aborts_if !(global<Account>(addr).guid_creation_num == 2);
        aborts_if exists<Configuration>(@lumio_framework);
        // property 1: During the module's initialization, it guarantees that the Configuration resource will move under
        // the Lumio framework account with initial values.
        /// [high-level-req-1]
        ensures exists<Configuration>(@lumio_framework);
    ...
    spec current_epoch(): u64 {
        aborts_if !exists<Configuration>(@lumio_framework);
        ensures result == global<Configuration>(@lumio_framework).epoch;
    }

    spec disable_reconfiguration(lumio_framework: &signer) {
        include AbortsIfNotLumioFramework;
        aborts_if exists<DisableReconfiguration>(@lumio_framework);
        ensures exists<DisableReconfiguration>(@lumio_framework);
    }

    /// Make sure the caller is admin and check the resource DisableReconfiguration.
    spec enable_reconfiguration(lumio_framework: &signer) {
        use lumio_framework::reconfiguration::{DisableReconfiguration};
        include AbortsIfNotLumioFramework;
        aborts_if !exists<DisableReconfiguration>(@lumio_framework);
        ensures !exists<DisableReconfiguration>(@lumio_framework);
    }
    ...
    spec emit_genesis_reconfiguration_event {
        use lumio_framework::reconfiguration::{Configuration};

        aborts_if !exists<Configuration>(@lumio_framework);
        let config_ref = global<Configuration>(@lumio_framework);
        aborts_if !(config_ref.epoch == 0 && config_ref.last_reconfiguration_time == 0);
        ensures global<Configuration>(@lumio_framework).epoch == 1;
    }

    spec last_reconfiguration_time {
        aborts_if !exists<Configuration>(@lumio_framework);
        ensures result == global<Configuration>(@lumio_framework).last_reconfiguration_time;
    }
    ...
    spec reconfigure {
        ...
        let success = !(chain_status::is_genesis() || timestamp::spec_now_microseconds() == 0 || !reconfiguration_enabled())
            && timestamp::spec_now_microseconds() != global<Configuration>(@lumio_framework).last_reconfiguration_time;
        include features::spec_periodical_reward_rate_decrease_enabled() ==> staking_config::StakingRewardsConfigEnabledRequirement;
        include success ==> lumio_coin::ExistsLumioCoin;
        aborts_if false;
        ...
        ensures success ==> global<Configuration>(@lumio_framework).epoch == old(global<Configuration>(@lumio_framework).epoch) + 1;
        ensures success ==> global<Configuration>(@lumio_framework).last_reconfiguration_time == timestamp::spec_now_microseconds();
        ...
        /// [high-level-req-4]
        /// [high-level-req-5]
        ensures !success ==> global<Configuration>(@lumio_framework).epoch == old(global<Configuration>(@lumio_framework).epoch);
    }
    ...
    spec reconfiguration_enabled {
        ...
        /// [high-level-req-2]
        aborts_if false;
        ensures result == !exists<DisableReconfiguration>(@lumio_framework);
    }
}
