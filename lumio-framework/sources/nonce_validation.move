module lumio_framework::nonce_validation {
    use lumio_std::table::{Self, Table};
    use lumio_std::timestamp;
    ...
    public(friend) fun initialize(lumio_framework: &signer) {
        initialize_nonce_table(lumio_framework);
    }

    public entry fun initialize_nonce_table(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        if (!exists<NonceHistory>(@lumio_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(lumio_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@lumio_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@lumio_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@lumio_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
