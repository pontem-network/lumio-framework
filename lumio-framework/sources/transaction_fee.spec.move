spec lumio_framework::transaction_fee {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, it guarantees that the Lumio framework signer may burn
    /// Lumio coins.
    /// Criticality: Critical
    /// Implementation: The LumioCoinCapabilities structure is defined in this module and it stores burn capability to
    /// burn the gas fees.
    ...
    /// Criticality: Critical
    /// Implementation: The initialize_fee_collection_and_distribution function ensures only the Lumio framework address
    /// calls it.
    ...
    ///
    /// No.: 6
    /// Requirement: The presence of the resource, indicating collected fees per block under the Lumio framework account,
    ...
    /// process_collected_fees all ensure that the CollectedFeesPerBlock resource exists under lumio_framework by
    ...
    spec module {
        ...
        // property 1: Given the blockchain is in an operating state, it guarantees that the Lumio framework signer may burn Lumio coins.
        /// [high-level-req-1]
        invariant [suspendable] chain_status::is_operating() ==> exists<LumioCoinCapabilities>(@lumio_framework) || exists<LumioFABurnCapabilities>(@lumio_framework);
    }
    ...
    spec initialize_fee_collection_and_distribution(_lumio_framework: &signer, _burn_percentage: u8) {
    }

    /// `LumioCoinCapabilities` should be exists.
    spec burn_fee(account: address, fee: u64) {
        use lumio_std::type_info;
        use lumio_framework::optional_aggregator;
        use lumio_framework::coin;
        use lumio_framework::coin::{CoinInfo, CoinStore};
        // TODO(fa_migration)
        pragma verify = false;

        aborts_if !exists<LumioCoinCapabilities>(@lumio_framework);

        // This function essentially calls `coin::burn_coin`, monophormized for `LumioCoin`.
        let account_addr = account;
        let amount = fee;

        let lumio_addr = type_info::type_of<LumioCoin>().account_address;
        let coin_store = global<CoinStore<LumioCoin>>(account_addr);
        let post post_coin_store = global<CoinStore<LumioCoin>>(account_addr);

        // modifies global<CoinStore<LumioCoin>>(account_addr);

        aborts_if amount != 0 && !(exists<CoinInfo<LumioCoin>>(lumio_addr)
            && exists<CoinStore<LumioCoin>>(account_addr));
        aborts_if coin_store.coin.value < amount;

        let maybe_supply = global<CoinInfo<LumioCoin>>(lumio_addr).supply;
        let supply_aggr = option::spec_borrow(maybe_supply);
        let value = optional_aggregator::optional_aggregator_value(supply_aggr);

        let post post_maybe_supply = global<CoinInfo<LumioCoin>>(lumio_addr).supply;
        ...
        ensures coin::supply<LumioCoin> == old(coin::supply<LumioCoin>) - amount;
    }
    ...
    spec mint_and_refund(account: address, refund: u64) {
        ...
        aborts_if (refund != 0) && !exists<CoinInfo<LumioCoin>>(lumio_addr);
        include coin::CoinAddAbortsIf<LumioCoin> { amount: refund };

        aborts_if !exists<CoinStore<LumioCoin>>(account);
        // modifies global<CoinStore<LumioCoin>>(account);

        aborts_if !exists<LumioCoinMintCapability>(@lumio_framework);

        let supply = coin::supply<LumioCoin>;
        let post post_supply = coin::supply<LumioCoin>;
        aborts_if [abstract] supply + refund > MAX_U128;
        ensures post_supply == supply + refund;
    }

    /// Ensure caller is admin.
    /// Aborts if `LumioCoinCapabilities` already exists.
    spec store_lumio_coin_burn_cap(lumio_framework: &signer, burn_cap: BurnCapability<LumioCoin>) {
        use std::signer;
        ...
        aborts_if !system_addresses::is_lumio_framework_address(addr);

        aborts_if exists<LumioFABurnCapabilities>(addr);
        aborts_if exists<LumioCoinCapabilities>(addr);

        ensures exists<LumioFABurnCapabilities>(addr) || exists<LumioCoinCapabilities>(addr);
    }

    /// Ensure caller is admin.
    /// Aborts if `LumioCoinMintCapability` already exists.
    spec store_lumio_coin_mint_cap(lumio_framework: &signer, mint_cap: MintCapability<LumioCoin>) {
        use std::signer;
        let addr = signer::address_of(lumio_framework);
        aborts_if !system_addresses::is_lumio_framework_address(addr);
        aborts_if exists<LumioCoinMintCapability>(addr);
        ensures exists<LumioCoinMintCapability>(addr);
    }
