spec lumio_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<LumioCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<LumioCoin> { account: staker };
        let balance = global<coin::CoinStore<LumioCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<LumioCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@lumio_framework);
        requires exists<stake::ValidatorSet>(@lumio_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @lumio_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<lumio_framework::timestamp::CurrentTimeMicroseconds>(@lumio_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@lumio_framework);
        aborts_if exists<stake::AllowedValidators>(@lumio_framework) && !contains(allowed.accounts, resource_addr);
