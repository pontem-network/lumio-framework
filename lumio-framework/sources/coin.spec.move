spec lumio_framework::coin {
    ...
    /// Can only be updated by `@lumio_framework`.
    spec allow_supply_upgrades(_lumio_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@lumio_framework)) {
            let map = global<CoinConversionMap>(@lumio_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@lumio_framework`.
    ...
    // `account` must be `@lumio_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@lumio_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@lumio_framework);
