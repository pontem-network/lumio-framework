...
module lumio_framework::lumio_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use lumio_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    use lumio_framework::reconfiguration_with_dkg;
    use lumio_framework::timestamp;
    use lumio_framework::voting;
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@lumio_framework)) {
            move_to(
                lumio_framework,
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@lumio_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(lumio_framework);
        initialize_partial_voting(lumio_framework);
        move_to(lumio_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(lumio_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(lumio_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(lumio_framework),
            vote_events: account::new_event_handle<VoteEvent>(lumio_framework),
        });
        move_to(lumio_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(lumio_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@lumio_framework);
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = borrow_global_mut<GovernanceEvents>(@lumio_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(lumio_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@lumio_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@lumio_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@lumio_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@lumio_framework);
    ...
    public fun get_remaining_voting_power(
        ...
        let used_voting_power = *VotingRecordsV2[@lumio_framework].votes.borrow_with_default(record_key, &0);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@lumio_framework);
        ...
        let total_voting_token_supply = coin::supply<LumioCoin>();
    ...
    fun vote_internal(
        ...
        let used_voting_power = VotingRecordsV2[@lumio_framework].votes.borrow_mut_with_default(record_key, 0);
        ...
        *used_voting_power += voting_power;
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = &mut GovernanceEvents[@lumio_framework];
            event::emit_event(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@lumio_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@lumio_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@lumio_framework, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<GovernanceProposal>(@lumio_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposal>(@lumio_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@lumio_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@lumio_framework).hashes;
    ...
    public entry fun reconfigure(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            reconfiguration_with_dkg::try_start();
        } else {
            reconfiguration_with_dkg::finish(lumio_framework);
        }
    }
    ...
    public entry fun force_end_epoch(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        reconfiguration_with_dkg::finish(lumio_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(lumio_framework: &signer) acquires GovernanceResponsbility {
        let core_signer = get_signer_testnet_only(lumio_framework, @0x1);
        system_addresses::assert_lumio_framework(&core_signer);
        reconfiguration_with_dkg::finish(&core_signer);
    }

    /// Update feature flags and also trigger reconfiguration.
    public fun toggle_features(lumio_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_lumio_framework(lumio_framework);
        features::change_feature_flags_for_next_epoch(lumio_framework, enable, disable);
        reconfigure(lumio_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(lumio_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@lumio_framework);
    ...
    fun assert_voting_initialization() {
        assert!(exists<VotingRecordsV2>(@lumio_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_partial_voting(&lumio_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(lumio_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&lumio_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@lumio_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @lumio_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @lumio_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@lumio_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@lumio_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(lumio_framework = @lumio_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(lumio_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(lumio_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(lumio_framework = @lumio_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code = 0x5000a, location = lumio_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(lumio_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(lumio_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<GovernanceProposal>(@lumio_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@lumio_framework, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<GovernanceProposal>(@lumio_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@lumio_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@lumio_framework).hashes;
    ...
    #[test(lumio_framework = @lumio_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65541, location = lumio_framework::lumio_governance)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&lumio_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(lumio_framework = @lumio_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65551, location = lumio_framework::lumio_governance)]
    public entry fun test_cannot_vote_for_expired_proposal(
        ...
        setup_partial_voting_with_initialized_stake(&lumio_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(lumio_framework = @lumio_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @0x345)]
    #[expected_failure(abort_code = 65539, location = lumio_framework::lumio_governance)]
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&lumio_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        ...
        test_resolving_proposal_generic(lumio_framework, true, execution_hash);
    }
    ...
    public entry fun test_batch_vote(
        ...
        features::change_feature_flags_for_testing(&lumio_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
        setup_partial_voting(&lumio_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        let voter_1_addr = signer::address_of(&voter_1);
        let voter_2_addr = signer::address_of(&voter_2);
    ...
    public fun setup_voting(
        ...
        staking_config::initialize_for_test(lumio_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(lumio_framework, 10, 100, 1000);
        store_signer_cap(
            lumio_framework,
            @lumio_framework,
            account::create_test_signer_cap(@lumio_framework),
        );
        ...
        stake::create_validator_set(lumio_framework, active_validators, pks);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(lumio_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(lumio_framework, 10, 100, 1000);
        store_signer_cap(
            lumio_framework,
            @lumio_framework,
    ...
    public fun setup_partial_voting_with_initialized_stake(
        ...
        setup_voting_with_initialized_stake(lumio_framework, proposer, yes_voter, no_voter);
    }
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&lumio_framework));
        initialize(&lumio_framework, 1, 2, 3);
        update_governance_config(&lumio_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@lumio_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = lumio_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(lumio_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
