spec lumio_framework::lumio_account {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the creation of an Lumio account the following rules should hold: (1) the authentication key
    /// should be 32 bytes in length, (2) an Lumio account should not already exist for that authentication key, and (3)
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Lumio account, the account should become registered to receive LumioCoin.
    /// Criticality: Critical
    ...
    /// exist; then, register LumioCoin for that account if a particular is unregistered before transferring the amount.
    /// Criticality: Critical
    /// Implementation: The transfer function checks if the recipient account exists. If the account does not exist,
    /// the function creates one and registers the account to LumioCoin if not registered.
    ...
    ///
    /// No.: 7
    /// Requirement: When performing a batch transfer of Lumio Coin and/or a batch transfer of a custom coin type, it
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @lumio_framework || auth_key == @lumio_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        include GuidAbortsIf<LumioCoin>;
        include WithdrawAbortsIf<LumioCoin>{from: source};
        include TransferEnsures<LumioCoin>;

        aborts_if exists<coin::CoinStore<LumioCoin>>(to) && global<coin::CoinStore<LumioCoin>>(to).frozen;
        /// [high-level-req-5]
        ensures exists<lumio_framework::account::Account>(to);
        ensures exists<coin::CoinStore<LumioCoin>>(to);
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        ...
        // aborts_if !account::spec_exists_at(addr);
        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<LumioCoin>(addr);
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<LumioCoin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @lumio_framework || recipients[i] == @lumio_token);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<LumioCoin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<LumioCoin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<LumioCoin>>(recipients[i]) && global<coin::CoinStore<LumioCoin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<LumioCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<LumioCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @lumio_framework || to == @lumio_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<lumio_framework::account::Account>(to);
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<LumioCoin>();
    }
