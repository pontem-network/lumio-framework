...
module lumio_framework::block {
    ...
    public(friend) fun initialize(lumio_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<CommitHistory>(lumio_framework, CommitHistory {
            max_capacity: 2000,
            next_idx: 0,
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(lumio_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(lumio_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@lumio_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@lumio_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@lumio_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@lumio_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@lumio_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@lumio_framework);
    ...
    public entry fun test_update_epoch_interval(lumio_framework: signer) acquires BlockResource {
        account::create_account_for_test(@lumio_framework);
        initialize(&lumio_framework, 1);
        assert!(borrow_global<BlockResource>(@lumio_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&lumio_framework, 2);
        assert!(borrow_global<BlockResource>(@lumio_framework).epoch_interval == 2, 1);
    }

    #[test(lumio_framework = @lumio_framework, account = @0x123)]
