...
module lumio_framework::storage_gas {
    ...
    public(friend) fun set_config(lumio_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_lumio_framework(lumio_framework);
        *borrow_global_mut<StorageGasConfig>(@lumio_framework) = config;
    }
    ...
    public fun initialize(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        assert!(
            !exists<StorageGasConfig>(@lumio_framework),
        ...
        move_to(lumio_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@lumio_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(lumio_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@lumio_framework);
        let gas = borrow_global_mut<StorageGas>(@lumio_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@lumio_framework);
