...
module lumio_framework::lumio_coin {
    use std::error;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    use lumio_framework::coin::{Self, BurnCapability, MintCapability};
    ...
    public(friend) fun initialize(lumio_framework: &signer): (BurnCapability<LumioCoin>, MintCapability<LumioCoin>) {
        ...
        // Lumio framework needs mint cap to mint coins to initial validators. This will be revoked once the validators
        // have been initialized.
        move_to(lumio_framework, MintCapStore { mint_cap });
    ...
    public(friend) fun destroy_mint_cap(lumio_framework: &signer) acquires MintCapStore {
        system_addresses::assert_lumio_framework(lumio_framework);
        let MintCapStore { mint_cap } = move_from<MintCapStore>(@lumio_framework);
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account LumioCoin for gas so it can execute system transactions.
        let coins = coin::mint<LumioCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<LumioCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<LumioCoin>(amount, mint_cap);
        coin::deposit<LumioCoin>(dst_addr, coins_minted);
    }
    ...
    public fun mint_lum_fa_for_test(amount: u64): FungibleAsset acquires MintCapStore {
        ...
        coin::coin_to_fungible_asset(
            coin::mint(
                amount,
                &borrow_global<MintCapStore>(@lumio_framework).mint_cap
            )
        )
    }
    ...
    public fun ensure_initialized_with_lum_fa_metadata_for_test() {
        ...
        if (!exists<MintCapStore>(@lumio_framework)) {
            ...
            let (burn_cap, mint_cap) = initialize(&lumio_framework);
    ...
    #[test_only]
    public fun initialize_for_test(lumio_framework: &signer): (BurnCapability<LumioCoin>, MintCapability<LumioCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(lumio_framework);
        let (burn_cap, mint_cap) = initialize(lumio_framework);
        coin::create_coin_conversion_map(lumio_framework);
        coin::create_pairing<LumioCoin>(lumio_framework);
    ...
    public fun initialize_for_test_without_aggregator_factory(
        lumio_framework: &signer
    ): (BurnCapability<LumioCoin>, MintCapability<LumioCoin>) {
        let (burn_cap, mint_cap) = initialize(lumio_framework);
        coin::create_coin_conversion_map(lumio_framework);
        coin::create_pairing<LumioCoin>(lumio_framework);
        (burn_cap, mint_cap)
