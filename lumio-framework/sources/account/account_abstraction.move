module lumio_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    use lumio_framework::permissioned_signer::is_permissioned_signer;
    #[test_only]
    use lumio_framework::account::create_account_for_test;
    #[test_only]
    use lumio_framework::auth_data;

    friend lumio_framework::transaction_validation;
    #[test_only]
    friend lumio_framework::account_abstraction_tests;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@lumio_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    public entry fun initialize(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        move_to(
            lumio_framework,
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @lumio_fungible_asset)
    }
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@lumio_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@lumio_framework].auth_functions
   }
    ...
    fun authenticate(
        ...
        // Returned signer MUST represent the same account address. Otherwise, it may break the invariant of Lumio blockchain!
        assert!(
