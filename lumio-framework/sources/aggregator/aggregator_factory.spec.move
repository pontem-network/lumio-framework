spec lumio_framework::aggregator_factory {
    use lumio_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the Lumio framework is the caller and that the
    /// AggregatorFactory resource will move under the Lumio framework account.
    /// Criticality: High
    ...
    /// resource transfers to the Lumio framework account.
    ...
    /// under the Lumio account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the Lumio
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the Lumio framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the Lumio framework
    /// address.
    ...
    spec initialize_aggregator_factory(lumio_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@lumio_framework);
    }
