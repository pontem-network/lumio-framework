spec lumio_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Lumio Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_lumio_framework function ensures that the provided signer belongs to the
    /// @lumio_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotLumioFramework).
    ///
    /// No.: 3
    ...
    spec assert_lumio_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotLumioFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @lumio_framework with error::PERMISSION_DENIED;
    }
