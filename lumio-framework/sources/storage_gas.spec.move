spec lumio_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@lumio_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@lumio_framework);
    }
    ...
    spec set_config(lumio_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(lumio_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@lumio_framework);
        aborts_if exists<StorageGas>(@lumio_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@lumio_framework);
        ensures exists<StorageGas>(@lumio_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@lumio_framework);
        aborts_if !exists<StorageGas>(@lumio_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@lumio_framework);
    }
