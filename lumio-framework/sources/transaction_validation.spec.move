spec lumio_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `lumio_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use lumio_framework::timestamp::{CurrentTimeMicroseconds};
        use lumio_framework::chain_id::{ChainId};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@lumio_framework);
        aborts_if !(timestamp::now_seconds() < txn_expiration_time);

        aborts_if !exists<ChainId>(@lumio_framework);
    ...
    /// `LumioCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `LumioCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use lumio_framework::coin::{CoinStore, CoinInfo};
        use lumio_framework::optional_aggregator;
        use lumio_framework::transaction_fee::{LumioCoinCapabilities, LumioCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<LumioCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<LumioCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);

        aborts_if !exists<CoinStore<LumioCoin>>(gas_payer);
        ...
        let total_supply = coin::supply<LumioCoin>;
        let post post_total_supply = coin::supply<LumioCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<LumioCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<LumioCoinMintCapability>(@lumio_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<LumioCoin>>(lumio_addr);
        include coin::CoinAddAbortsIf<LumioCoin> { amount: amount_to_mint };
    }
}
