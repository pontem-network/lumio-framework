...
module lumio_framework::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;
    ...
    friend lumio_framework::consensus_config;
    friend lumio_framework::execution_config;
    friend lumio_framework::gas_schedule;
    friend lumio_framework::genesis;
    friend lumio_framework::version;
    ...
    /// Publishes `Configuration` resource. Can only be invoked by lumio framework account, and only a single time in Genesis.
    public(friend) fun initialize(lumio_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(lumio_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            lumio_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(lumio_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(lumio_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(lumio_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_lumio_framework(lumio_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(lumio_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@lumio_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@lumio_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@lumio_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@lumio_framework).epoch
    }
