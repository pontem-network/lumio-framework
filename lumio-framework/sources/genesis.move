module lumio_framework::genesis {
    use std::error;
    ...
    use lumio_framework::create_signer::create_signer;
    use lumio_framework::gas_schedule;
    use lumio_framework::nonce_validation;
    use lumio_framework::reconfiguration;
    use lumio_framework::stake;
    use lumio_framework::staking_contract;
    use lumio_framework::staking_config;
    use lumio_framework::state_storage;
    use lumio_framework::storage_gas;
    ...
    fun initialize(
        ...
        // Initialize the lumio framework account. This is the account where system resources and modules will be
        // deployed to. This will be entirely managed by on-chain governance and no entities have the key or privileges
        // to use this account.
        let (lumio_framework_account, lumio_framework_signer_cap) = account::create_framework_reserved_account(@lumio_framework);
        // Initialize account configs on lumio framework account.
        account::initialize(&lumio_framework_account);
        ...
        lumio_governance::store_signer_cap(&lumio_framework_account, @lumio_framework, lumio_framework_signer_cap);
        ...
        while (!vector::is_empty(&framework_reserved_addresses)) {
            ...
            lumio_governance::store_signer_cap(&lumio_framework_account, address, framework_signer_cap);
        };

        consensus_config::initialize(&lumio_framework_account, consensus_config);
        execution_config::set(&lumio_framework_account, execution_config);
        version::initialize(&lumio_framework_account, initial_version);
        stake::initialize(&lumio_framework_account);
        stake::initialize_pending_transaction_fee(&lumio_framework_account);
        timestamp::set_time_has_started(&lumio_framework_account);
        ...
        storage_gas::initialize(&lumio_framework_account);
        gas_schedule::initialize(&lumio_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&lumio_framework_account);

        chain_id::initialize(&lumio_framework_account, chain_id);
        reconfiguration::initialize(&lumio_framework_account);
        block::initialize(&lumio_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&lumio_framework_account);
        nonce_validation::initialize(&lumio_framework_account);
    }

    /// Genesis step 2: Initialize Lumio coin.
    fun initialize_lumio_coin(lumio_framework: &signer) {
        let (burn_cap, mint_cap) = lumio_coin::initialize(lumio_framework);

        coin::create_coin_conversion_map(lumio_framework);
        coin::create_pairing<LumioCoin>(lumio_framework);

        // Give stake module MintCapability<LumioCoin> so it can mint rewards.
        stake::store_lumio_coin_mint_cap(lumio_framework, mint_cap);
        // Give transaction_fee module BurnCapability<LumioCoin> so it can burn gas.
        transaction_fee::store_lumio_coin_burn_cap(lumio_framework, burn_cap);
        // Give transaction_fee module MintCapability<LumioCoin> so it can mint refunds.
        transaction_fee::store_lumio_coin_mint_cap(lumio_framework, mint_cap);
    }

    /// Only called for testnets and e2e tests.
    fun initialize_core_resources_and_lumio_coin(
    ...
    fun create_accounts(lumio_framework: &signer, accounts: vector<AccountMap>) {
    ...
    fun create_account(lumio_framework: &signer, account_address: address, balance: u64): signer {
        ...
        if (coin::balance<LumioCoin>(account_address) == 0) {
            coin::register<LumioCoin>(&account);
            lumio_coin::mint(lumio_framework, account_address, balance);
        };
        account
    }
    ...
    fun create_employee_validators(
        ...
        vector::for_each_ref(&employees, |employee_group| {
            ...
            while (j < num_employees_in_group) {
                ...
                let total = coin::balance<LumioCoin>(*account);
                let coins = coin::withdraw<LumioCoin>(&employee, total);
    ...
    fun create_initialize_validators_with_commission(
        ...
        vector::for_each_ref(&validators, |validator| {
            ...
            create_initialize_validator(lumio_framework, validator, use_staking_contract);
        });

        // Destroy the lumio framework account's ability to mint coins now that we're done with setting up the initial
        // validators.
        lumio_coin::destroy_mint_cap(lumio_framework);
    ...
    fun create_initialize_validators(lumio_framework: &signer, validators: vector<ValidatorConfiguration>) {
        ...
        create_initialize_validators_with_commission(lumio_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(lumio_framework, validator.owner_address, validator.stake_amount);
        create_account(lumio_framework, validator.operator_address, 0);
        create_account(lumio_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(lumio_framework: &signer) {
        chain_status::set_genesis_end(lumio_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        features::change_feature_flags_for_verification(lumio_framework, vector[1, 2], vector[]);
        initialize_lumio_coin(lumio_framework);
        lumio_governance::initialize_for_verification(
            lumio_framework,
            min_voting_threshold,
            required_proposer_stake,
            voting_duration_secs
        );
        create_accounts(lumio_framework, accounts);
        create_employee_validators(employee_vesting_start, employee_vesting_period_duration, employees);
        create_initialize_validators_with_commission(lumio_framework, true, validators);
        set_genesis_end(lumio_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@lumio_framework), 1);
    ...
    fun test_create_account(lumio_framework: &signer) {
        setup();
        initialize_lumio_coin(lumio_framework);

        let addr = @0x121341; // 01 -> 0a are taken
        let test_signer_before = create_account(lumio_framework, addr, 15);
        let test_signer_after = create_account(lumio_framework, addr, 500);
        assert!(test_signer_before == test_signer_after, 0);
        assert!(coin::balance<LumioCoin>(addr) == 15, 1);
    }

    #[test(lumio_framework = @0x1)]
    fun test_create_accounts(lumio_framework: &signer) {
        setup();
        initialize_lumio_coin(lumio_framework);
        ...
        create_accounts(lumio_framework, accounts);
        assert!(coin::balance<LumioCoin>(addr0) == 12345, 0);
        assert!(coin::balance<LumioCoin>(addr1) == 67890, 1);

        create_account(lumio_framework, addr0, 23456);
        assert!(coin::balance<LumioCoin>(addr0) == 12345, 2);
    }

    #[test(lumio_framework = @0x1, root = @0xabcd)]
    fun test_create_root_account(lumio_framework: &signer) {
        use lumio_framework::aggregator_factory;
