spec lumio_framework::vesting {
    ...
    /// period.
    /// Enforcement: Audited the check for the end of vesting cliff: [https://github.com/lumio-labs/lumio-core/blob/main/lumio-move/framework/lumio-framework/sources/vesting.move#L566](vest) module.
    ///
    /// No.: 5
    ...
    spec create_vesting_contract {
        ...
        pragma verify = false;
        /// [high-level-req-10]
        aborts_if withdrawal_address == @lumio_framework || withdrawal_address == @vm_reserved;
        aborts_if !exists<account::Account>(withdrawal_address);
        aborts_if !exists<coin::CoinStore<LumioCoin>>(withdrawal_address);
    ...
    spec set_beneficiary(
        ...
        // TODO(fa_migration)
        // aborts_if !coin::spec_is_account_registered<LumioCoin>(new_beneficiary);
        include VerifyAdminAbortsIf;
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<LumioCoin>>(resource_addr) && !lumio_std::type_info::spec_is_struct<LumioCoin>();
        aborts_if !exists<coin::CoinStore<LumioCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 > MAX_U64;
        aborts_if !exists<coin::CoinStore<LumioCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        ensures exists<account::Account>(resource_addr) && post_acc.authentication_key == account::ZERO_AUTH_KEY &&
                exists<coin::CoinStore<LumioCoin>>(resource_addr);
    ...
    spec withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<LumioCoin> {
        // TODO: Calls `staking_contract::distribute` which is not verified.
        pragma verify = false;
    ...
    spec schema DistributeInternalAbortsIf {
        ...
        aborts_if !exists<stake::ValidatorSet>(@lumio_framework);
        let validator_set = global<stake::ValidatorSet>(@lumio_framework);
