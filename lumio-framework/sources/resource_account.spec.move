spec lumio_framework::resource_account {
    /// <high-level-req>
    /// No.: 1
    ...
    spec create_resource_account_and_fund(
        ...
        let source_addr = signer::address_of(origin);
        let resource_addr = account::spec_create_resource_address(source_addr, seed);
        let coin_store_resource = global<coin::CoinStore<LumioCoin>>(resource_addr);

        include lumio_account::WithdrawAbortsIf<LumioCoin>{from: origin, amount: fund_amount};
        include lumio_account::GuidAbortsIf<LumioCoin>{to: resource_addr};
        include RotateAccountAuthenticationKeyAndStoreCapabilityAbortsIfWithoutAccountLimit;

        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<LumioCoin>(resource_addr) && coin_store_resource.frozen;
        /// [high-level-req-3]
        ensures exists<lumio_framework::coin::CoinStore<LumioCoin>>(resource_addr);
    }
    ...
    spec rotate_account_authentication_key_and_store_capability(
        ...
        ensures vector::length(optional_auth_key) != 0 ==>
            global<lumio_framework::account::Account>(resource_addr).authentication_key == optional_auth_key;
    }
