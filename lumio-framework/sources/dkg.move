...
module lumio_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use lumio_framework::event::emit;
    use lumio_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@lumio_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@lumio_framework)) {
            borrow_global<DKGState>(@lumio_framework).in_progress
        } else {
            option::none()
        }
    }
