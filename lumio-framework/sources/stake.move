...
module lumio_framework::stake {
    ...
    use lumio_std::math64::min;
    use lumio_std::big_ordered_map::{Self, BigOrderedMap};
    ...
    use lumio_framework::coin::{Self, Coin, MintCapability};
    use lumio_framework::event::{Self, EventHandle};
    use lumio_framework::timestamp;
    use lumio_framework::system_addresses;
    use lumio_framework::staking_config::{Self, StakingConfig, StakingRewardsConfig};
    ...
    friend lumio_framework::reconfiguration_with_dkg;
    friend lumio_framework::transaction_fee;
    ...
    struct StakePool has key {
        // active stake
        active: Coin<LumioCoin>,
        // inactive stake, can be withdrawn
        inactive: Coin<LumioCoin>,
        // pending activation for next epoch
        pending_active: Coin<LumioCoin>,
        // pending deactivation for next epoch
        pending_inactive: Coin<LumioCoin>,
    ...
    struct ValidatorFees has key {
        fees_table: Table<address, Coin<LumioCoin>>,
    }
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@lumio_framework);
    ...
    public fun get_current_epoch_proposal_counts(validator_index: u64): (u64, u64) acquires ValidatorPerformance {
        let validator_performances = &borrow_global<ValidatorPerformance>(@lumio_framework).validators;
    ...
    public fun get_pending_transaction_fee(): vector<u64> acquires PendingTransactionFee {
        ...
        let fee_table = &borrow_global<PendingTransactionFee>(@lumio_framework).pending_fee_by_validator;
    ...
    public(friend) fun initialize(lumio_framework: &signer) {
        system_addresses::assert_lumio_framework(lumio_framework);

        move_to(lumio_framework, ValidatorSet {
            consensus_scheme: 0,
        ...
        move_to(lumio_framework, ValidatorPerformance {
    ...
    public(friend) fun store_lumio_coin_mint_cap(lumio_framework: &signer, mint_cap: MintCapability<LumioCoin>) {
        system_addresses::assert_lumio_framework(lumio_framework);
        move_to(lumio_framework, LumioCoinCapabilities { mint_cap })
    }
    ...
    public fun remove_validators(
        ...
        system_addresses::assert_lumio_framework(lumio_framework);
        let validator_set = borrow_global_mut<ValidatorSet>(@lumio_framework);
    ...
    public fun initialize_pending_transaction_fee(framework: &signer) {
        system_addresses::assert_lumio_framework(framework);

        if (!exists<PendingTransactionFee>(@lumio_framework)) {
    ...
    public fun set_transaction_fee_config(framework: &signer, config: TransactionFeeConfig) acquires TransactionFeeConfig {
        ...
        if (exists<TransactionFeeConfig>(@lumio_framework)) {
            *borrow_global_mut<TransactionFeeConfig>(@lumio_framework) = config;
        } else {
    ...
    public(friend) fun record_fee(
        ...
        let pending_fee = borrow_global_mut<PendingTransactionFee>(@lumio_framework);
        let i = 0;
    ...
    fun initialize_owner(owner: &signer) acquires AllowedValidators {
        ...
        move_to(owner, StakePool {
            active: coin::zero<LumioCoin>(),
            pending_active: coin::zero<LumioCoin>(),
            pending_inactive: coin::zero<LumioCoin>(),
            inactive: coin::zero<LumioCoin>(),
            locked_until_secs: 0,
    ...
    public entry fun add_stake(owner: &signer, amount: u64) acquires OwnerCapability, StakePool, ValidatorSet {
        ...
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
        add_stake_with_cap(ownership_cap, coin::withdraw<LumioCoin>(owner, amount));
    }

    /// Add `coins` into `pool_address`. this requires the corresponding `owner_cap` to be passed in.
    public fun add_stake_with_cap(owner_cap: &OwnerCapability, coins: Coin<LumioCoin>) acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        ...
        if (is_current_epoch_validator(pool_address)) {
            coin::merge<LumioCoin>(&mut stake_pool.pending_active, coins);
        } else {
            coin::merge<LumioCoin>(&mut stake_pool.active, coins);
        };
    ...
    public fun unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        coin::merge<LumioCoin>(&mut stake_pool.pending_inactive, unlocked_stake);
    ...
    public entry fun withdraw(
        ...
        coin::deposit<LumioCoin>(owner_address, coins);
    }
    ...
    public fun withdraw_with_cap(
        ...
        withdraw_amount: u64
    ): Coin<LumioCoin> acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (withdraw_amount == 0) return coin::zero<LumioCoin>();
    ...
    public(friend) fun update_performance_statistics(
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@lumio_framework);
    ...
    public(friend) fun on_new_epoch(
    ) acquires LumioCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@lumio_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@lumio_framework);
        ...
        if (exists<PendingTransactionFee>(@lumio_framework)) {
            let pending_fee_by_validator = &mut borrow_global_mut<PendingTransactionFee>(@lumio_framework).pending_fee_by_validator;
    ...
    public fun next_validator_consensus_infos(): vector<ValidatorConsensusInfo> acquires ValidatorSet, ValidatorPerformance, StakePool, ValidatorConfig {
        // Init.
        let cur_validator_set = borrow_global<ValidatorSet>(@lumio_framework);
        let staking_config = staking_config::get();
        let validator_perf = borrow_global<ValidatorPerformance>(@lumio_framework);
    ...
    fun update_stake_pool(
    ...
    ) acquires LumioCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
        ...
        let fee_active = 0;
        let fee_limit = if (exists<TransactionFeeConfig>(@lumio_framework)) {
            let TransactionFeeConfig::V0 { max_fee_octa_allowed_per_epoch_per_pool } = borrow_global<TransactionFeeConfig>(@lumio_framework);
        ...
        if (std::features::is_distribute_transaction_fee_enabled()) {
            let mint_cap = &borrow_global<LumioCoinCapabilities>(@lumio_framework).mint_cap;
            if (fee_active > 0) {
    ...
    fun distribute_rewards(
        stake: &mut Coin<LumioCoin>,
        num_successful_proposals: u64,
        num_total_proposals: u64,
        rewards_rate: u64,
        rewards_rate_denominator: u64,
    ): u64 acquires LumioCoinCapabilities {
    ...
    public fun configure_allowed_validators(
        ...
        let lumio_framework_address = signer::address_of(lumio_framework);
        system_addresses::assert_lumio_framework(lumio_framework);
        if (!exists<AllowedValidators>(lumio_framework_address)) {
            move_to(lumio_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(lumio_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@lumio_framework)) {
            true
        } else {
            let allowed = borrow_global<AllowedValidators>(@lumio_framework);
    ...
    use lumio_std::bls12381::proof_of_possession_from_bytes;
    use lumio_framework::reconfiguration_state;
    use lumio_framework::validator_consensus_info;
    use lumio_framework::validator_consensus_info::ValidatorConsensusInfo;
    #[test_only]
    ...
    public fun initialize_for_test(lumio_framework: &signer) {
        reconfiguration_state::initialize(lumio_framework);
        initialize_for_test_custom(lumio_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
        // In the test environment, the periodical_reward_rate_decrease feature is initially turned off.
        features::change_feature_flags_for_testing(lumio_framework, vector[], vector[features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration_state::initialize(lumio_framework);
        if (!exists<ValidatorSet>(@lumio_framework)) {
            initialize(lumio_framework);
        };
        ...
        if (!exists<LumioCoinCapabilities>(@lumio_framework)) {
            let (burn_cap, mint_cap) = lumio_coin::initialize_for_test(lumio_framework);
            store_lumio_coin_mint_cap(lumio_framework, mint_cap);
    ...
    // This function assumes the stake module already the capability to mint lumio coins.
    #[test_only]
    public fun mint_coins(amount: u64): Coin<LumioCoin> acquires LumioCoinCapabilities {
        let mint_cap = &borrow_global<LumioCoinCapabilities>(@lumio_framework).mint_cap;
        coin::mint(amount, mint_cap)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires LumioCoinCapabilities {
    ...
    public fun mint_and_add_stake(
        account: &signer, amount: u64) acquires LumioCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, LumioCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(lumio_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(lumio_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(lumio_framework = @lumio_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        ...
        assert!(coin::balance<LumioCoin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<LumioCoin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<LumioCoin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(lumio_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(lumio_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(lumio_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(lumio_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        let (_sk_1, pk_1, pop_1) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@lumio_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@lumio_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<LumioCoin>(validator_address) == 1001, 2);
    ...
    #[test(lumio_framework = @lumio_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(lumio_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(lumio_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@lumio_framework).pending_inactive,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@lumio_framework).pending_active,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@lumio_framework).active_validators,
                0
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(lumio_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        end_epoch();
        let vci_vec_0 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@lumio_framework));
        ...
        assert!(vector[@0x5, @lumio_framework, @0x3] == vci_addrs, 1);
        ...
        let vci_vec_1 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@lumio_framework));
        assert!(vci_vec_0 == vci_vec_1, 11);
        join_validator_set(validator_2, v2_addr);
        let vci_vec_2 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@lumio_framework));
        assert!(vci_vec_0 == vci_vec_2, 12);
        leave_validator_set(validator_1, v1_addr);
        let vci_vec_3 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@lumio_framework));
        assert!(vci_vec_0 == vci_vec_3, 13);
        join_validator_set(validator_4, v4_addr);
        let vci_vec_4 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@lumio_framework));
    ...
    public entry fun test_validator_rewards_rate_decrease_over_time(
        ...
        features::change_feature_flags_for_testing(lumio_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);
    ...
    public entry fun test_invalid_config(
        lumio_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, LumioCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(lumio_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@lumio_framework).active_validators) == 2, 0);
        ...
        remove_validators(lumio_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@lumio_framework).active_validators) == 1, 0);
        assert!(get_validator_state(validator_to_remove) == VALIDATOR_STATUS_PENDING_INACTIVE, 1);
    }

    #[test(vm = @0x0, lumio_framework = @0x1, validator_0 = @0x123, validator_1 = @0x234)]
    public entry fun test_transaction_fee(
        ...
        initialize_for_test(lumio_framework);
        initialize_pending_transaction_fee(lumio_framework);
        features::change_feature_flags_for_testing(lumio_framework, vector[features::get_distribute_transaction_fee_feature()], vector[]);
    ...
    public entry fun test_transaction_fee_limit(
...

        {
            ...
            set_transaction_fee_config(lumio_framework, config);
            end_epoch();
    ...
    #[test(lumio_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        lumio_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        let (burn, mint) = lumio_coin::initialize_for_test(lumio_framework);
        configure_allowed_validators(lumio_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(lumio_framework, vector[]);
        let (burn, mint) = lumio_coin::initialize_for_test(lumio_framework);
