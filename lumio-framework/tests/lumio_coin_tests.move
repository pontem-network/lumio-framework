...
module lumio_framework::lumio_coin_tests {
    ...
    public fun mint_lum_fa_to_for_test<T: key>(store: Object<T>, amount: u64) {
        fungible_asset::deposit(store, lumio_coin::mint_lum_fa_for_test(amount));
    }

    public fun mint_lum_fa_to_primary_fungible_store_for_test(
        owner: address,
        amount: u64,
    ) {
        primary_fungible_store::deposit(owner, lumio_coin::mint_lum_fa_for_test(amount));
    }

    #[test(lumio_framework = @lumio_framework)]
    fun test_lum_setup_and_mint(lumio_framework: &signer) {
    ...
    fun test_fa_helpers_for_test() {
        ...
        assert!(object::object_exists<Metadata>(@lumio_fungible_asset), 0);
        mint_lum_fa_to_primary_fungible_store_for_test(@lumio_framework, 100);
        ...
        mint_lum_fa_to_for_test(object::address_to_object<FungibleStore>(store_addr), 100);
