spec aptos_framework::resource_account {
    ...
    spec create_resource_account_and_fund(
        ...
        let source_addr = signer::address_of(origin);
        let resource_addr = account::spec_create_resource_address(source_addr, seed);
        let coin_store_resource = global<coin::CoinStore<NativeCoin>>(resource_addr);

        include aptos_account::WithdrawAbortsIf<NativeCoin>{from: origin, amount: fund_amount};
        include aptos_account::GuidAbortsIf<NativeCoin>{to: resource_addr};
        include RotateAccountAuthenticationKeyAndStoreCapabilityAbortsIfWithoutAccountLimit;

        //coin property
        aborts_if coin::is_account_registered<NativeCoin>(resource_addr) && coin_store_resource.frozen;
        ensures exists<aptos_framework::coin::CoinStore<NativeCoin>>(resource_addr);
    }
