module aptos_framework::aptos_account {
    ...
    public entry fun create_account(auth_key: address) {
        ...
        coin::register<NativeCoin>(&signer);
    }
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (!coin::is_account_registered<NativeCoin>(to)) {
            coin::register<NativeCoin>(&create_signer(to));
        };
        coin::transfer<NativeCoin>(source, to, amount)
    }
    ...
    public fun deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) acquires DirectTransferConfig {
        if (!account::exists_at(to)) {
            create_account(to);
            spec {
                assert coin::is_account_registered<NativeCoin>(to);
                assume aptos_std::type_info::type_of<CoinType>() == aptos_std::type_info::type_of<NativeCoin>() ==>
    ...
    public fun assert_account_is_registered_for_apt(addr: address) {
        ...
        assert!(coin::is_account_registered<NativeCoin>(addr), error::not_found(EACCOUNT_NOT_REGISTERED_FOR_APT));
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = aptos_framework::native_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<NativeCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<NativeCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<NativeCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        assert!(!coin::is_account_registered<NativeCoin>(resource_acc_addr), 0);

        let (burn_cap, mint_cap) = aptos_framework::native_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<NativeCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<NativeCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<NativeCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
