spec aptos_framework::aptos_account {
    ...
    spec create_account(auth_key: address) {
        ...
        ensures exists<coin::CoinStore<NativeCoin>>(auth_key);
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        let coin_store_to = global<coin::CoinStore<NativeCoin>>(to);

        // The 'from' addr is implictly not equal to 'to' addr
        requires account_addr_source != to;

        include CreateAccountTransferAbortsIf;
        include GuidAbortsIf<NativeCoin>;
        include WithdrawAbortsIf<NativeCoin>{from: source};

        aborts_if exists<coin::CoinStore<NativeCoin>>(to) && global<coin::CoinStore<NativeCoin>>(to).frozen;
        ensures exists<aptos_framework::account::Account>(to);
        ensures exists<coin::CoinStore<NativeCoin>>(to);
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        aborts_if !account::exists_at(addr);
        aborts_if !coin::is_account_registered<NativeCoin>(addr);
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<NativeCoin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<NativeCoin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<NativeCoin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<NativeCoin>>(recipients[i]) && global<coin::CoinStore<NativeCoin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<NativeCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<NativeCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<NativeCoin>()
            && !coin::is_account_registered<CoinType>(to) && !can_receive_direct_coin_transfers(to);
    }
}
