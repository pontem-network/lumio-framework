...
module aptos_framework::native_coin {
    use std::string;
    use std::error;
    use std::signer;
    use std::vector;
    ...
    struct NativeCoin has key {}

    struct MintCapStore has key {
        mint_cap: MintCapability<NativeCoin>,
    }
    ...
    public(friend) fun initialize(aptos_framework: &signer): (BurnCapability<NativeCoin>, MintCapability<NativeCoin>) {
        system_addresses::assert_aptos_framework(aptos_framework);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize_with_parallelizable_supply<NativeCoin>(
            aptos_framework,
            string::utf8(b"Native Coin"),
            string::utf8(b"NAT"),
            8, // decimals
            true, // monitor_supply
        );
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account NativeCoin for gas so it can execute system transactions.
        coin::register<NativeCoin>(core_resources);
        let coins = coin::mint<NativeCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<NativeCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<NativeCoin>(amount, mint_cap);
        coin::deposit<NativeCoin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    public fun initialize_for_test(aptos_framework: &signer): (BurnCapability<NativeCoin>, MintCapability<NativeCoin>) {
    ...
    #[test_only]
    public fun initialize_for_test_without_aggregator_factory(aptos_framework: &signer): (BurnCapability<NativeCoin>, MintCapability<NativeCoin>) {
        initialize(aptos_framework)
    }
}
