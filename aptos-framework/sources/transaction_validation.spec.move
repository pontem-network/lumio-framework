spec aptos_framework::transaction_validation {
    ...
    spec schema PrologueCommonAbortsIf {
        ...
        aborts_if !(txn_sequence_number == global<Account>(transaction_sender).sequence_number);
        aborts_if !exists<CoinStore<NativeCoin>>(gas_payer);
        // property 1: The sender of a transaction should have sufficient coin balance to pay the transaction fee.
        aborts_if !(global<CoinStore<NativeCoin>>(gas_payer).coin.value >= max_transaction_fee);
    }
    ...
    /// `NativeCoinCapabilities` and `CoinInfo` should exists.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use aptos_framework::transaction_fee::{NativeCoinCapabilities, CollectedFeesPerBlock};
        ...
        aborts_if !(global<CoinStore<NativeCoin>>(gas_payer).coin.value >= transaction_fee_amount);

        aborts_if !exists<Account>(addr);
        aborts_if !(global<Account>(addr).sequence_number < MAX_U64);

        let pre_balance = global<coin::CoinStore<NativeCoin>>(gas_payer).coin.value;
        let post balance = global<coin::CoinStore<NativeCoin>>(gas_payer).coin.value;
        ...
        let apt_addr = type_info::type_of<NativeCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<NativeCoin>>(apt_addr).supply;
        ...
        aborts_if if (features::spec_is_enabled(features::COLLECT_AND_DISTRIBUTE_GAS_FEES)) {
            !exists<CollectedFeesPerBlock>(@aptos_framework)
                ...
                ( // `exists<CoinStore<NativeCoin>>(addr)` checked above.
            ...
            // Sufficiency of funds is checked above.
            !exists<NativeCoinCapabilities>(@aptos_framework) ||
                // Existence of APT's CoinInfo
                transaction_fee_amount > 0 && !exists<CoinInfo<NativeCoin>>(aptos_addr) ||
        ...
        let post post_maybe_apt_supply = global<CoinInfo<NativeCoin>>(apt_addr).supply;
