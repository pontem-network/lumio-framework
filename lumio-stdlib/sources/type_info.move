module lumio_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `lumio_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `lumio_framework::chain_id::get()` will
    /// return whichever ID was passed to `lumio_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::lumio_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(type_info.account_address() == @lumio_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags_for_testing(&fx, vector[features::get_lumio_stdlib_chain_id_feature()], vector[]);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @lumio_std;
